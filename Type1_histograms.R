...
stanfit_joint <- stan("joint_fit3.stan",data=standatajoint,iter=10000,warmup=2000)
...
for(i in indeces ){
  ...
  stanfit_ind <- stan("fit_x_err_y_uppbound.stan",data = standata,iter=4000,warmup=2000)
...
for(index in orderyerr_1){
  ...
  #We scale all distribution functions

  m <- min( min(sd_ind[index,]), yerr[index] )
  scale <- ((2*pi)^(1/2)*m)^(-1)
  ...
  p <- ggplot(data=df)+
    ...
    geom_function(fun=fun,size=2.4,aes(color="Yobs Type 1"))+
    xlim(yobs[index]-3*yerr[index],yobs[index]+3*yerr[index])+xlab("logMBH")+
    ggtitle(paste(data_MBH[["galaxynames"]][index],"Type 1 observation",sep=", "))+
    ...
    scale_color_manual(name="Color legend",values=c("Yobs Type 1"="black","Yreal LogMassk"="brown",
